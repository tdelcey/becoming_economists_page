...
#| eval: TRUE
#| label: tbl-data
#| tbl-cap: Sample of metadata and person tables 
...
# Sample data from thesis_person
person_sample <- thesis_person %>% 
...
person_table <- datatable(person_sample, options = list(pageLength = 5,
                                                        searching = FALSE,
                                                        lengthChange = FALSE), caption = " Persons")
...
htmltools::browsable(
  htmltools::tagList(
    htmltools::tags$div(style = "display: flex; gap: 20px;",
      ...
      htmltools::tags$div(style = "flex: 1;", person_table)
    )
  )
)
...
saveRDS(parsed, here(FR_blog_data_path, "parsed.rds"), compress = TRUE)

# FILTER TOKENS 

parsed <- readRDS(here(FR_blog_data_path, "parsed.rds"))
...
saveRDS(term_list, here(FR_blog_data_path, "term_list.rds"))

# PREPARE STM INPUT 
term_list <- readRDS(here(FR_blog_data_path, "term_list.rds"))
...
saveRDS(corpus_in_stm, here(FR_blog_data_path, "corpus_in_stm.rds"))
```
...
corpus_in_stm <- readRDS(here(FR_blog_data_path, "corpus_in_stm.rds"))
...
saveRDS(stm, here(FR_blog_data_path, "stm.rds"))
```
...
stm <- readRDS(here(FR_blog_data_path, "stm.rds"))
...
saveRDS(estimate_effect, here(FR_blog_data_path, "estimate_effect.rds"))
```
...
estimate_effect <-readRDS(here(FR_blog_data_path, "estimate_effect.rds"))
...
saveRDS(corr_table, here(FR_blog_data_path, "corr_table.rds"))

saveRDS(graph_layout, here(FR_blog_data_path, "graph_layout.rds"))
```
...
graph_layout <- readRDS(here(FR_blog_data_path, "graph_layout.rds"))
...
corr_table <- readRDS(here(FR_blog_data_path, "corr_table.rds"))
