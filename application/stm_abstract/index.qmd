...
To run our topic model, we use the abstracts (`abstract_fr`) as the main textual input. As previously demonstrated [here](/application/stm_title#fig-textual_data), abstracts provide significantly better data than titles for training a topic model; however, they are only available from the early 1980s. Notably, this period also coincides with a sharp increase in the proportion of female authors.
...
@fig-regression_interaction presents the results of such an analysis. For each `year_defence` and `gender_expanded`, we compute the expected topic prevalence of topic 35, 53 and 92. Topics 35 and 53 are examples of topic prevalence strongly associated with women and men, respectively. Topic 92 provides a more interesting insight: while this topic is generally associated with male authors, it was mostly prevalent in women's theses until the mid-1990s.
